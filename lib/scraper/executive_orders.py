# Imports
from presidency.models import ExecutiveOrder, db
...
class ExecutiveOrderScraper:
	...
	def scrape():
		...
		Retrieve and store all new Executive Orders.
		"""
		...
		for year in years:
			url = "http://www.presidency.ucsb.edu/executive_orders.php?year=%d&Submit=DISPLAY" % year
			...
			for i in range(1, len(table)):
				...
				try:
					order = ExecutiveOrder(obj['pid'], obj['title'], obj['text'], obj['link'], datetime.datetime.strptime(obj['date'], "%B %d, %Y"), obj['president'])
					db.session.add(order)
					db.session.commit()
					print("Executive Order retrieved: %s" % obj['pid'])
				except:
					print("Executive Order %s not persisted." % obj['pid'])
